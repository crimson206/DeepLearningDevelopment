...
class Logger():
    def __init__(self):
        ...
        self.batch_values_temp_dict = {}
        self.batch_values_dict = {}

    @property
    def batch_dict(self):
        value_dict = {}
        for key, losses in self.batch_values_temp_dict.items():
            value_dict[key] = losses[-1] if losses else None
        return value_dict

    def batch_step(self, values, labels):
        for loss, label in zip(values, labels):
            if label in self.batch_values_temp_dict.keys():
                self.batch_values_temp_dict[label].append(loss)
            else:
                self.batch_values_temp_dict[label] = [loss]
    ...
    def epoch_step(self):
        for label, values in self.batch_values_temp_dict.items():
